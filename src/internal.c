...
int glslview_init(int argc, const char** argv){
	...
	if(usingopencl == 0){
		...
		if(glslview_create_shader(&shaders[0].shader, vertex, quadfrag, NULL, NULL, NULL) == 0){
			fprintf(stderr, "Invalid shader.\n");
			status = EXIT_FAILURE;
			return status;
		}else{
			glslview_update_shader_uniform(&shaders[0].uniform, &shaders[0].shader, displaymode.w, displaymode.h);
		}

		glslview_cl_resize(displaymode.w, displaymode.h);
